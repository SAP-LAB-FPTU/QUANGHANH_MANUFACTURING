            ...
            else
            {
                ...
                catch
                {
                    using (DbContextTransaction transaction = DBContext.Database.BeginTransaction())
                    {
                        try
                        {
                            List<Documentary_revoke_details> details = DBContext.Documentary_revoke_details.Where(x => x.documentary_id == documentary_id).ToList();
                            foreach (Documentary_revoke_details item in details)
                            {
