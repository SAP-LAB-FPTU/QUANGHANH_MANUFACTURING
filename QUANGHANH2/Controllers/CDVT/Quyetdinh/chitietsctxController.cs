    ...
    public class chitietsctxController : Controller
    {
        ...
        public ActionResult Detail()
        {
            ...
            using (QUANGHANHABCEntities db = new QUANGHANHABCEntities())
            {
                ...
                int count = 1;
                List<Documentary_big_maintain_detailsDB> documentariesList = (from a in db.Documentary_big_maintain_details
                                                                         ...
                                                                         }).ToList().Select(p => new Documentary_big_maintain_detailsDB
                                                                         {
                                                                             ...
                                                                             equipment_big_maintain_reason = p.equipment_big_maintain_reason,
                                                                             next_remodel_type = p.next_remodel_type,
                                                                             reason = p.reason,
                                                                             next_end_time = p.next_end_time
                                                                         }).ToList();
                ...
                //sorting
                documentariesList = documentariesList.OrderBy(sortColumnName + " " + sortDirection).ToList<Documentary_big_maintain_detailsDB>();
                //paging

                documentariesList = documentariesList.Skip(start).Take(length).ToList<Documentary_big_maintain_detailsDB>();
