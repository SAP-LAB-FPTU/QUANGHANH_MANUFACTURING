...
namespace QUANGHANHCORE.Controllers.CDVT.Quyetdinh
{
    public class ThanhlyController : Controller
    {
        ...
        [Route("phong-cdvt/quyet-dinh/thanh-ly/edit")]
        [HttpPost]
        ...
        public ActionResult Search(string documentary_code, string person_created, string dateStart, string dateEnd)
        {
            ...
            if (documentary_code == "" || documentary_code == null)
            {
                incidents = (from document in db.Documentaries
                             where document.documentary_type.Equals("5") && (document.documentary_code.Contains(documentary_code) || document.documentary_code == null) && document.person_created.Contains(person_created) && (document.date_created >= dtStart && document.date_created <= dtEnd)
            ...
            {
                incidents = (from document in db.Documentaries
                             where document.documentary_type.Equals("5") && (document.documentary_code.Contains(documentary_code)) && document.person_created.Contains(person_created) && (document.date_created >= dtStart && document.date_created <= dtEnd)
                             join detail in db.Documentary_liquidation_details on document.documentary_id equals detail.documentary_id
                             into temporary
                             select new
                             {
        ...
        public void ExportExcel()
        {
            ...
            using (ExcelPackage excelPackage = new ExcelPackage(file))
            {
                ...
                using (QUANGHANHABCEntities db = new QUANGHANHABCEntities())
                {
                    ...
                    excelPackage.SaveAs(new FileInfo(location + "/ThanhLyThietBi.xlsx"));
                }
