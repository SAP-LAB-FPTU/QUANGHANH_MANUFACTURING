...
//            string sql = "update Documentary.RepairDetails set isVisible = 0 where documentary_id = @iddoc and equipmentId = @id";
...
//            //list vat tu di kem
//            List<Supply_DK> listsupdk = DBContext.Database.SqlQuery<Supply_DK>("select equipment_name from Equipment.Equipment where isAttach = 1").ToList();
//            ViewBag.listsupdk = listsupdk;
//            //list vat tu di kem cua thiet bi
//            List<vtdk> listsupdktb = DBContext.Database.SqlQuery<vtdk>("select s.equipmentId_dikem, e.equipment_name from Equipment.Equipment e join dbo.Supply_DiKem s on e.equipmentId = s.equipmentId_dikem where s.equipmentId = @id", new SqlParameter("id", id)).ToList();
//            ViewBag.listtbdk = listsupdktb;
//            List<Dikem_Vattu> listdkvt = DBContext.Database.SqlQuery<Dikem_Vattu>("select s.equipmentId_dikem, e.equipment_name, su.supply_id, su.supply_name, ss.quantity from Equipment.Equipment e join dbo.Supply_DiKem s on e.equipmentId = s.equipmentId join dbo.Supply_Equipment_DiKem ss on s.equipmentId_dikem = ss.equipmentId join Supply.Supply su on ss.supply_id = su.supply_id where e.equipmentId = @id", new SqlParameter("id", id)).ToList();
//            ViewBag.dikemvattu = listdkvt;
//            //NK su co
//            var years = DBContext.Database.SqlQuery<int>("SELECT distinct year(i.start_time) as years FROM Equipment.Incident i inner join Equipment.Equipment e on e.equipmentId = i.equipmentId inner join General.Department d on d.department_id = i.department_id where i.end_time is not null and e.equipmentId = @id order by years desc", new SqlParameter("id", id)).ToList();
...
//                var dates = DBContext.Database.SqlQuery<DateTime>("SELECT distinct i.start_time as dates FROM Equipment.Incident i inner join Equipment.Equipment e on e.equipmentId = i.equipmentId inner join General.Department d on d.department_id = i.department_id where i.end_time is not null and e.equipmentId = @id and year(start_time) = @year order by dates desc", new SqlParameter("id", id), new SqlParameter("year", year)).ToList();
...
//                        "i.* FROM Equipment.Incident i inner join Equipment.Equipment e on e.equipmentId = i.equipmentId inner join General.Department d " +
...
//            //DD thiet bi
//            var equipment = DBContext.Database.SqlQuery<EquipWithName>("SELECT e.*,d.department_name,s.statusname FROM Equipment.Equipment e LEFT JOIN Equipment.Status s on e.current_Status = s.statusid LEFT JOIN General.Department d ON d.department_id = e.department_id WHERE e.equipmentId = @id", new SqlParameter("id", id)).FirstOrDefault();
//            ViewBag.equipment = equipment;
//            List<ddplus> listddplus = new List<ddplus>();
//            var car = DBContext.Database.SqlQuery<Car>("select * from Equipment.Car where equipmentId = @id", new SqlParameter("id", id)).FirstOrDefault();
//            if (car != null)
//            {
...
//            //thiet bi di kem
//            var sup = DBContext.Database.SqlQuery<Supply_DK>("select s.*,e.equipment_name from Equipment.Equipment e join dbo.Supply_DiKem s on e.equipmentId = s.equipmentId_dikem where s.equipmentId = @id", new SqlParameter("id", equipment.equipmentId)).ToList();
//            ViewBag.sup = sup;
//            //vat tu di kem
//            var supVTDK = DBContext.Database.SqlQuery<VT_DK>("select s.*,e.equipment_name,sp.supply_name from Equipment.Equipment e join dbo.Vattu_Dikem s on e.equipmentId = s.equipmentId join Supply.Supply sp on sp.supply_id = s.supply_id where s.equipmentId = @id", new SqlParameter("id", equipment.equipmentId)).ToList();
//            ViewBag.supVTDK = supVTDK;
//            //Vat tu SCTX
//            var supSCTX = DBContext.Database.SqlQuery<Supply_DP>("select s.*,e.supply_name from Supply.Supply e join Supply.RepairRegularly s on e.supply_id = s.supply_id where s.equipmentId = @id", new SqlParameter("id", equipment.equipmentId)).ToList();
//            ViewBag.supSCTX = supSCTX;
//            //Vat tu DP
//            var supDP = DBContext.Database.SqlQuery<Supply_DK>("select s.*,e.equipment_name from dbo.Supply_DiKem s join Equipment.Equipment e on s.equipmentId_dikem = e.equipmentId where s.equipmentId = @id", new SqlParameter("id", equipment.equipmentId)).ToList();
//            ViewBag.supDP = supDP;
//            //NK kiem dinh
//            years = DBContext.Database.SqlQuery<int>("SELECT distinct year(ei.inspect_date) as years FROM Equipment.Inspection ei inner join Equipment.Equipment e on e.equipmentId = ei.equipmentId where ei.inspect_date is not null and e.equipmentId = @equipmentId order by years desc",
//                new SqlParameter("equipmentId", id)).ToList();
//            List<Inspection> EI = DBContext.Database.SqlQuery<Inspection>("SELECT * FROM Equipment.Inspection WHERE inspect_date IS NOT NULL AND equipmentId = @equipmentId",
//                new SqlParameter("equipmentId", id)).ToList();
...
//            //NK bao hiem
//            years = DBContext.Database.SqlQuery<int>("SELECT distinct year(ei.insurance_start_date) as years FROM Equipment.Insurance ei inner join Equipment.Equipment e on e.equipmentId = ei.equipmentId where  e.equipmentId = @equipmentId order by years desc",
//                new SqlParameter("equipmentId", id)).ToList();
//            List<Equipment_InsDB> EIs = DBContext.Database.SqlQuery<Equipment_InsDB>("SELECT * FROM Equipment.Insurance WHERE equipmentId = @equipmentId order by insurance_start_date desc",
//                new SqlParameter("equipmentId", id)).ToList();
...
//                            join s in DBContext.RepairRegularlies on e.equipmentId equals s.equipmentId
...
//            {
//                DBContext.Configuration.LazyLoadingEnabled = false;
//                List<myFuel> listFuel = DBContext.Database.SqlQuery<myFuel>("select f.*, d.department_name from Equipment.FuelActivitiesConsumption f, Equipment.Equipment e, General.Department d where e.equipmentId = f.equipmentId and e.department_id =  d.department_id and e.equipmentId = @id order by f.date desc", new SqlParameter("id", id)).ToList();
...
//            {
//                DBContext.Configuration.LazyLoadingEnabled = false;
//                List<myAct> listHD = DBContext.Database.SqlQuery<myAct>("select a.*,d.department_name from Equipment.Activity a, Equipment.Equipment e, General.Department d where a.equipmentid = e.equipmentId and e.department_id = d.department_id and a.equipmentid = @id", new SqlParameter("id", id)).ToList();
...
//                try
//                {
//                    string sql = "delete from dbo.Supply_DiKem where equipmentId_dikem = @supid and equipmentId = @eid";
//                    DBContext.Database.ExecuteSqlCommand(sql, new SqlParameter("supid", supid), new SqlParameter("eid", id));
//                    DBContext.SaveChanges();
//                    dbc.Commit();
//                    var sup = DBContext.Database.SqlQuery<Supply_DK>("select s.*,e.equipment_name from Equipment.Equipment e join dbo.Supply_DiKem s on e.equipmentId = s.equipmentId_dikem where s.equipmentId = @id", new SqlParameter("id", id)).ToList();
...
//                try
//                {
//                    string sql = "update dbo.Supply_DiKem set quantity = @quan, note = @note where equipmentId_dikem = @supid and equipmentId = @eid";
...
//                try
//                {
//                    string sql = "update dbo.Supply_DiKem set quantity_duphong = @quan, note = @note where equipmentId_dikem = @supid and equipmentId = @eid";
//                    DBContext.Database.ExecuteSqlCommand(sql, new SqlParameter("quan", quan), new SqlParameter("note", note), new SqlParameter("supid", supid), new SqlParameter("eid", id));
//                    DBContext.SaveChanges();
//                    dbc.Commit();
//                    var sup = DBContext.Database.SqlQuery<Supply_DK>("select s.*,e.equipment_name from Equipment.Equipment e join dbo.Supply_DiKem s on e.equipmentId = s.equipmentId_dikem where s.equipmentId = @id", new SqlParameter("id", id)).ToList();
...
//                    DBContext.SaveChanges();
//                    dbc.Commit();
//                    var sup = DBContext.Database.SqlQuery<QUANGHANH2.Models.Attribute>("select * from Equipment.Attribute e where e.equipmentId = @id", new SqlParameter("id", eid)).ToList();
//                    return Json(new { success = true, data = sup }, JsonRequestBehavior.AllowGet);
//                }
//                catch (Exception e)
//                {
...
//                                    sql_sup += "insert into dbo.Supply_Equipment_DiKem values ('" + s.supply_id + "', @eid, " + quan[i] + ") ";
//                                    break;
//                                }
//                            }
//                        }
...
//                    DBContext.SaveChanges();
//                    dbc.Commit();
//                    var sup = DBContext.Database.SqlQuery<Dikem_Vattu>("select s.equipmentId_dikem, e.equipment_name, su.supply_id, su.supply_name, ss.quantity from Equipment.Equipment e join dbo.Supply_DiKem s on e.equipmentId = s.equipmentId join dbo.Supply_Equipment_DiKem ss on s.equipmentId_dikem = ss.equipmentId join Supply.Supply su on ss.supply_id = su.supply_id where e.equipmentId = @id", new SqlParameter("id", id_e)).ToList();
//                    return Json(new { success = true, data = sup }, JsonRequestBehavior.AllowGet);
//                }
//                catch (Exception e)
//                {
...
//                try
//                {
//                    string sql = "update dbo.Supply_Equipment_DiKem set quantity = @quan where supply_id = @supid and equipmentId = @eid";
...
//                try
//                {
//                    string sql = "delete from dbo.Supply_Equipment_DiKem where supply_id = @supid and equipmentId = @eid";
//                    DBContext.Database.ExecuteSqlCommand(sql, new SqlParameter("supid", supid), new SqlParameter("eid", id));
//                    DBContext.SaveChanges();
//                    dbc.Commit();
//                    var sup = DBContext.Database.SqlQuery<Dikem_Vattu>("select s.equipmentId_dikem, e.equipment_name, su.supply_id, su.supply_name, ss.quantity from Equipment.Equipment e join dbo.Supply_DiKem s on e.equipmentId = s.equipmentId join dbo.Supply_Equipment_DiKem ss on s.equipmentId_dikem = ss.equipmentId join Supply.Supply su on ss.supply_id = su.supply_id where e.equipmentId = @id", new SqlParameter("id", id_e)).ToList();
...
//                    DBContext.SaveChanges();
//                    dbc.Commit();
//                    var supVTDK = DBContext.Database.SqlQuery<VT_DK>("select s.*,e.equipment_name,sp.supply_name from Equipment.Equipment e join dbo.Vattu_Dikem s on e.equipmentId = s.equipmentId join Supply.Supply sp on sp.supply_id = s.supply_id where s.equipmentId = @id", new SqlParameter("id", id)).ToList();
...
//                try
//                {
//                    string sql = "update dbo.Vattu_Dikem set quantity = @quan where supply_id = @supid and equipmentId = @eid";
//                    DBContext.Database.ExecuteSqlCommand(sql, new SqlParameter("quan", quan), new SqlParameter("supid", supid), new SqlParameter("eid", id));
//                    DBContext.SaveChanges();
//                    dbc.Commit();
//                    var supVTDK = DBContext.Database.SqlQuery<VT_DK>("select s.*,e.equipment_name,sp.supply_name from Equipment.Equipment e join dbo.Vattu_Dikem s on e.equipmentId = s.equipmentId join Supply.Supply sp on sp.supply_id = s.supply_id where s.equipmentId = @id", new SqlParameter("id", id)).ToList();
...
//                try
//                {
//                    string sql = "delete from dbo.Vattu_Dikem where supply_id = @supid and equipmentId = @eid";
//                    DBContext.Database.ExecuteSqlCommand(sql, new SqlParameter("supid", supid), new SqlParameter("eid", id));
//                    DBContext.SaveChanges();
//                    dbc.Commit();
//                    var supVTDK = DBContext.Database.SqlQuery<VT_DK>("select s.*,e.equipment_name,sp.supply_name from Equipment.Equipment e join dbo.Vattu_Dikem s on e.equipmentId = s.equipmentId join Supply.Supply sp on sp.supply_id = s.supply_id where s.equipmentId = @id", new SqlParameter("id", id)).ToList();
...
//                            }
//                        }
//                        string sql_sup = "insert into Supply.RepairRegularly(supply_id, equipmentId, quantity) values (@supid, @eid, @quan)";
//                        DBContext.Database.ExecuteSqlCommand(sql_sup
//                            , new SqlParameter("@supid", s.supply_id)
//                            , new SqlParameter("@eid", id)
//                            , new SqlParameter("@quan", quan));
...
//                    DBContext.SaveChanges();
//                    dbc.Commit();
//                    var sup = DBContext.Database.SqlQuery<Supply_DP>("select e.*,s.supply_name from Supply.RepairRegularly e join Supply.Supply s on e.supply_id = s.supply_id where e.equipmentId = @id", new SqlParameter("id", id)).ToList();
//                    return Json(new { success = true, data = sup }, JsonRequestBehavior.AllowGet);
//                }
//                catch (Exception e)
//                {
...
//                try
//                {
//                    string sql = "update Supply.RepairRegularly set quantity = @quan where supply_id = @supid and equipmentId = @eid";
//                    DBContext.Database.ExecuteSqlCommand(sql, new SqlParameter("quan", quan), new SqlParameter("supid", supid), new SqlParameter("eid", id));
//                    DBContext.SaveChanges();
//                    dbc.Commit();
//                    var sup = DBContext.Database.SqlQuery<Supply_DP>("select e.*,s.supply_name from Supply.RepairRegularly e join Supply.Supply s on e.supply_id = s.supply_id where e.equipmentId = @id", new SqlParameter("id", id)).ToList();
...
//                try
//                {
//                    string sql = "delete from Supply.RepairRegularly where supply_id = @supid and equipmentId = @eid";
//                    DBContext.Database.ExecuteSqlCommand(sql, new SqlParameter("supid", supid), new SqlParameter("eid", id));
//                    DBContext.SaveChanges();
//                    dbc.Commit();
//                    var sup = DBContext.Database.SqlQuery<Supply_DP>("select e.*,s.supply_name from Supply.RepairRegularly e join Supply.Supply s on e.supply_id = s.supply_id where e.equipmentId = @id", new SqlParameter("id", id)).ToList();
