...
namespace QUANGHANHCORE.Controllers.CDVT
{
    ...
    public class DashController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var temp = db.Database.SqlQuery<Temp>("select distinct e.equipmentId from Equipment e where e.current_Status = N'Đang sửa chữa'").ToList<Temp>();
            etk.total_repair = temp.Count().ToString();
            temp = db.Database.SqlQuery<Temp>("select distinct e.equipmentId from Equipment e where e.current_Status = N'Đang bảo dưỡng'").ToList<Temp>();
            ...
            temp = db.Database.SqlQuery<Temp>("select distinct e.equipmentId from Equipment e where e.current_Status = N'Đang thanh lý'").ToList<Temp>();
            etk.total_TL = temp.Count().ToString();
            temp = db.Database.SqlQuery<Temp>("select distinct e.equipmentId from Equipment e where e.current_Status = N'Chờ điều động'").ToList<Temp>();
            ...
            List<DashEquip> listRepair = db.Database.SqlQuery<DashEquip>("select e.equipmentId,e.equipment_name from Equipment e where e.current_Status = N'Đang sửa chữa'").ToList();
            ViewBag.listRepair = listRepair;

            List<DashEquip> listMain = db.Database.SqlQuery<DashEquip>("select e.equipmentId,e.equipment_name from Equipment e where e.current_Status = N'Đang bảo dưỡng'").ToList();
            ...
            List<DashEquip> listTL = db.Database.SqlQuery<DashEquip>("select e.equipmentId,e.equipment_name from Equipment e where e.current_Status = N'Đang thanh lý'").ToList();
            ViewBag.listTL = listTL;

            List<DashEquip> listTH = db.Database.SqlQuery<DashEquip>("select e.equipmentId,e.equipment_name from Equipment e where e.current_Status = N'Chờ điều động'").ToList();
