...
namespace QUANGHANH2.Controllers
{
    public class NotifiController : Controller
    {
        ...
        public JsonResult CDVT()
        {
            ...
            var hanDangKiem = db.Equipments.Where(x => x.duration_of_inspection <= testTimeKD && x.duration_of_inspection >= DateTime.Now).OrderBy(x => x.duration_of_inspection).
                                    Select(x => new main
                                    {
                                        type = 0,
                                        pb = "Cơ điện vận tải",
                                        title = "Mã thiết bị " + x.equipment_id,
                                        name = x.equipment_name,
                                        date = x.duration_of_inspection.Value
                                    }).ToList();
            var hanBaohiem = (from equip in db.Equipments.Where(x => x.duration_of_insurance <= testTime && x.duration_of_insurance >= DateTime.Now)
                              select new main
                              {
                                  type = 1,
                                  pb = "Cơ điện vận tải",
                                  title = "Mã thiết bị " + equip.equipment_id,
                                  name = equip.equipment_name,
                                  date = equip.duration_of_insurance.Value
        ...
        public ActionResult Export(int type)
        {
            ...
            using (ExcelPackage excelPackage = new ExcelPackage(file))
            {
                ...
                using (QuangHanhManufacturingEntities db = new QuangHanhManufacturingEntities())
                {
                    ...
                    if (type == 0)
                    {
                        var maintitle = db.Equipments.Where(x => x.duration_of_inspection <= testTimeKD && x.duration_of_inspection >= DateTime.Now).OrderBy(x => x.duration_of_inspection).
                                                Select(x => new main
                                                {
                                                    type = 0,
                                                    pb = "Cơ điện vận tải",
                                                    title = x.equipment_id,
                                                    name = x.equipment_name,
                                                    date = x.duration_of_inspection.Value
                                                }).ToList().Distinct();
                    ...
                    {
                        var testTime = DateTime.Now.AddDays(10);
                        var maintitle = (from equip in db.Equipments.Where(x => x.duration_of_insurance <= testTime && x.duration_of_insurance >= DateTime.Now)
                                         join cate in db.CategoryAttributes.Where(x => x.equipment_category_attribute_name == "Số máy" || x.equipment_category_attribute_name == "Số khung")
                                            on equip.equipment_category_id equals cate.equipment_category_id
                                         select new main
                                         {
                                             type = 1,
                                             pb = "Cơ điện vận tải",
                                             title = equip.equipment_id,
                                             name = equip.equipment_name,
                                             date = equip.duration_of_insurance.Value
                                         }).OrderBy(x => x.date).ToList();
                        foreach (main item in maintitle)
